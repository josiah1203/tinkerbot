...
Paid plans have **no seat or repository cap**. Entitlements (SSO, SCIM, Change Sets, audit export, hosted API, factory improvement, premium routing) are calculated from the in-source catalog plus billing status. Stripe JSON may contain only Price IDs and `catalogVersion`.
