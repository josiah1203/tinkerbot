...
The supported local analysis commands are `check`, `test-integrity`, `impact`, `report`, `doctor`, `config validate`, `config explain`, `usage`, `baseline init|check|update`, `policy list|explain`, `artifacts`, `select-tests`, `contracts`, `fixtures`, and `history|history compare`. Empty `tb` prints help. `tb tui` is a tabbed master terminal on a TTY (`pnpm tb tui`); `--once` prints one Node check transcript. `tb agents` lists PATH CLIs. `tb factory new` writes a local starter tree. `tb dashboard` opens the hosted app. `--help` and `--version` are available without a repository analysis.

`login`, `logout`, `whoami`, `org list`, `org switch <organization-id>`, `org seats`, `billing summary|catalog|portal`, `factory list|show|validate|sync|mcp`, `work *`, `cell list`, `product list|show`, `skill list|show`, `evolution list|show|approve`, `run show|logs`, `receipt validate`, and `verify --repository owner/repository` use an HTTPS control-plane URL and session from `tb login` or environment variables. `tb factory sync` uploads `factory.yaml` plus the `.tinkerbot/` tree. `github run` and `serve` remain recognized but unshipped and return exit code `12`.
